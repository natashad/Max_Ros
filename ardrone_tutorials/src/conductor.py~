...
from terpischore.msg import pair
...
class Conductor(object):
    ...
    def determine_state(self, cond):
        ...
        if cond.data(0) == 4: # or the tempo is too fast
            ...
            if t < 2.0:
                ...
                self.z = 16.0*D*math.pow(t-3.5,3)-12.0*D*pow(t-3.5,2)+D
                self.vz = 48.0*D*math.pw(t-3.5,2)-24.0*D*(t-3.5)
        ...
        if cond.data(0) == 2:
            ...
            if t < 1.0:
                self.z = (A/2.0)*(1.0 - cos(2.0*math.pi*t))
                self.vz = A*math.pi*math.sin(2.0*math.pi*t)
                self.az = 2.0*A*math.pi*math.pi*math.cos(2.0*math.pi*t)
            else:
                self.z = (B/2.0)*(1.0 - cos(2.0*math.pi*t))
            ...
            if t < 1.5:
                self.y = (R/2.0)*(1.0 - cos((4.0/3.0)*math.pi*t))
